    ...
    public partial class Form1 : Form
    {
        ...
        private void InitializeFirebase(string credentialPath)
        {
            try
            {
                ...
                AddLog("Kimlik dosyasý ayarlandý");

                _db = FirestoreDb.Create("valorantagentcontrol-1");
                AddLog("Firebase veritabaný baðlantýsý oluþturuldu");
                ...
                AddLog("Test baðlantýsý baþarýlý");
        ...
        private async void ConnectButton_Click(object? sender, EventArgs e)
        {
            if (!isConnected && pathTextBox != null)
            {
                try
                {
                    ...
                    AddLog("Firebase'e baþarýyla baðlandý");
                }
                catch (Exception ex)
                {
                    AddLog($"Baðlantý hatasý: {ex.Message}");
            ...
            else
            {
                try
                {
                    ...
                    {
                        app.Delete();
                        AddLog("Firebase baðlantýsý kesildi");
                    }
                }
                catch { }
        ...
        private async Task StartListening()
        {
            ...
            AddLog("Komut dinlemeye baþlandý");
            ...
            var listener = collection.Listen(snapshot =>
            {
                foreach (var change in snapshot.Changes)
                {
                        ...
                        change.ChangeType == DocumentChange.Type.Modified)
                    {
                        ...
                        AddLog($"Yeni komut alýndý: {command.Action}, X: {command.X}, Y: {command.Y}");
        ...
        private void ProcessCommand(Command command)
        {
            ...
            // Debug için ham veriyi yazdýralým
            var rawData = command.ToString();
            AddLog($"Ham veri: {rawData}");
            AddLog($"Komut detaylarý - Action: {command.Action}, X: {command.X}, Y: {command.Y}");
        ...
        private void ClickAtPosition(int x, int y)
        {
            try
            {
                AddLog($"Fare týklamasý baþlatýlýyor: X: {x}, Y: {y}");
                SetCursorPos(x, y);
                mouse_event(MOUSEEVENTF_LEFTDOWN, x, y, 0, 0);
                System.Threading.Thread.Sleep(100);
                mouse_event(MOUSEEVENTF_LEFTUP, x, y, 0, 0);
                AddLog($"Fare týklamasý tamamlandý: X: {x}, Y: {y}");
            }
            catch (Exception ex)
            {
                AddLog($"Fare týklama hatasý: {ex.Message}");
            }
        }
    }
